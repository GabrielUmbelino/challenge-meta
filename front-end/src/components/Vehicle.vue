        ...
        <v-container>
          <v-flex xs12>
            ...
            <v-card>
              ...
              <v-layout row>
                <v-flex xs12 sm6>
                  <v-text-field v-model="vehicle.chassisId.chassisSeries" :disabled="isEditing" label="Chassis Series"></v-text-field>
                </v-flex>
                <v-flex xs12 sm6>
                  <v-text-field v-model="vehicle.chassisId.chassisNumber" :disabled="isEditing" label="Chassis Number"></v-text-field>
                </v-flex>
              </v-layout>
              <v-layout row>
                <v-flex xs12 sm6>
                   <v-select v-model="vehicle.type" :items="Object.keys(types)" :disabled="isEditing" @change="() => vehicle.numberOfPassengers = types[vehicle.type].passengers" label="Standard" ></v-select>
                </v-flex>
                <v-flex xs12 sm6>
                  <v-text-field :value="vehicle.numberOfPassengers" disabled label="Standard" ></v-text-field>
                </v-flex>
              </v-layout>
              <v-layout row>
                <v-flex xs12 sm6>
                  <v-text-field v-model="vehicle.color" label="Color"></v-text-field>
                </v-flex>
              </v-layout>
              <v-card-actions>
                <v-layout row>
